...
lo tienes instalado, corre el siguiente comando en tu terminal:
...
aplicación y corre el siguiente comando:
...
- La carpeta `app/` es un contenedor para la mayor parte de tu proyecto.
  Allí es dónde pondrás tus páginas y tus rutas API.
...
- La carpeta `app/core/` es el lugar principal para poner tus componentes,
  hooks, etcétera que son utilizados a lo largo de tu aplicación.

- `db/` es dónde la configuración de tu base de datos se aloja. Si
...
Ahora, asegúrate de estás dentro del directorio `my-blitz-app` y corre el
siguiente comando:
...
Eso significa que el servidor de desarrollo está corriendo. Ve a
...
¡Excelentes noticias, una base de tados SQLite ya ha sido creada para ti!
Puedes correr `blitz prisma studio` en la terminal y verás una interfaz
web donde podrás ver los datos de tu base de datos.
...
Blitz provee un conveniente comando llamado [`generate`](cli-generate)
para crear código base para luego escribir nuestras necesidades. Usaremos
...
Y cuando te lo pregunten, presiona **Enter** para correr `prisma migrate`
y actualizar el esquema de tu base de datos. Te preguntará por un nombre
para la migración; pon algo como "question creada".
...
Y luego, corre `blitz prisma generate` para actualizar el cliente de
...
Por el momento, Prisma no soporta soporta la "eliminación en cascada". En
el contexto de este tutorial, significa que las `Choice` no se borrar
...
¡Increíble! Ahora corre tu aplicación. Para ejecutarla, corre `blitz dev`
en tu terminal y ve a `localhost:3000/questions`. Prueba creando
preguntas, editándolas y eliminándolas.
...
Puedes ir a `localhost:3000/question/new` y crear una nueva pregunta con
opciones.
...
1. Reemplaza `<h1>Question {question.id}</h1>` with
   `<h1>{question.text}</h1>`.
...
🥳 ¡Felicitaciones! Has creado tu primera app con Blitz. Diviértete
jugando con ella, o compartíendosela a tus amigos. Ahora que terminaste
este tutorial, ¿por qué no intentas mejorar tu app aún más? Puedes probar:
...
Visita [discord.blitzjs.com](https://discord.blitzjs.com). Luego, comparte
el link en el canal **#built-with-blitz** para que todos puedan verla!
