...
Si obtienes un error, ejecuta `blitz prisma format`.

Fíjate que esta no requiere una migración de la base de datos, porque aún
no hemos añadido el campo `Choice` al modelo `Question`. Por ende,
elegiremos `false` cuando nos pregunte por hacer una migración.
...
Ahora podemos ejecutar la migración para actualizar nuestra base de datos:

```sh
blitz prisma migrate dev
```

Y nuevamente, elige un nombre para la migración, como _"add choice"_:
...
Ahora, nuestra base de datos está lista y el cliente de Prisma ha sido
generado. ¡Sigamos y juguemos con Prisma!
...
export function QuestionForm<S extends z.ZodType<any, any>>(
  ...
  return (
    <Form<S> {...props}>
      <LabeledTextField name="text" label="Texto" placeholder="Texto" />
+     <LabeledTextField name="choices.0.text" label="Opción 1" />
+     <LabeledTextField name="choices.1.text" label="Opción 2" />
+     <LabeledTextField name="choices.2.text" label="Opción 3" />
    </Form>
  )
}
```
...
+   } catch (error) {
+     alert(
+       "Error actualizando la opción " + JSON.stringify(error, null, 2)
+     )
+   }
+ }
...
export const Question = () => {
  ...
  return (
    <>
      <Head>
        <title>Pregunta {question.id}</title>
      </Head>
...
Para que `yarn tsc` o `git push` funcionen correctamente, necesitarás
borrar el archivo `app/choices/mutations/createChoice.ts` (inutilizado) o
actualizar el esquema CreateChoice para incluir los campos requeridos.
