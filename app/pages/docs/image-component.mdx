...
penalizada por imágenes que están fuera de ventana. Las imágenes se
cargarán cuando aparezcan en la pantalla.
...
function Home() {
  return (
    <>
      <h1>Mi Página Web</h1>
      <Image src={profilePic} alt="Foto del autor" />
      <p>¡Bienvenid@s a mi página web!</p>
    </>
  )
}
...
## Importar imágenes {#imports}

Puedes importar imágenes que vivan dentro de tu proyecto utilizando
`import`. (Nota: `require` no está soportado.)

Al hacer `import` directamente, `width`, `height` y `blurDataURL` serán
proporcionados automáticamente al componente `<Image>`. El texto
alternativo aún debe ser provisto por separado.
  ...
  return (
    <>
      ...
      <Image
        ...
        //
        // Permite añadir una versión difuminada de la imagen
        // mientras esta carga (opcional)
        // placeholder="blur"
      />
      <p>¡Bienvenid@s a mi página web!</p>
    </>
...
Para imágenes remotas o dinámicas, necesitarás proporcionar `width`,
`height`, y `blurDataURL` manualmente.
...
utilizar el optimizador de Blitz, puedes configurar qué proveedor y qué
prefijo utilizar. El prefijo te permite usar URL relativas en `src`.
...
A continuación se describe el algoritmo de cacheo para el
...
pida una imagen cuyo caché haya expirado, este caché será borrado y se
generará la imagen nuevamente.
...
que navegan tu sitio, puedes especificar una lista de puntos de cortes (o
_breakpoints_) usando la propiedad `deviceSizes`. Estos anchos son
utilizados cuando el componente `Image` usa `layout="responsive"` o
`layout="fill"` para que la imagen correcta sea entregada al dispositivo
visitando tu sitio.
...
### Desactivar las importaciones estáticas {#disable-static-imports}

El comportamiento por defecto te permite importar archivos estáticos como
`import icon from './icon.png` y pasárselos a la propiedad `src`.

En algunos casos, tal vez desees desactivar esta función para evitar
conflictos con otros paquetes o _plugins_ que tengas instalados.

Puedes desactivar las importaciones de imágenes estáticas con la siguiente
configuración:
...
PoDemostracións servir una imagen optimizada de este modo:
...
### Propiedades obligatorias {#required-props}

El componente `Image` necesita las siguientes propiedades.

#### src

Obligatorio y debe ser uno de los siguientes:

1. Una imagen importada estáticamente, como en los ejemplos de más arriba.
2. Una ruta como texto. Esta puede ser una URL externa o un ruta interna
   dependiendo del [cargador](#loader).
...
Obligatorio, excepto para imágenes importadas estáticamente o cuando
[`layout="fill"`](#layout).
...
El disposición (o _layout_) de la imagen cuando la ventana (o _viewport_)
cambia de tamaño. Su valor por defecto es `intrinsic`.

Cuando es `fixed`, las dimensiones de la imagen no cambian aunque el
tamaño de la ventana varíe (no es _responsive_), similar a como funciona
el elemento `img`.
...
- [Demostración de `fixed`](https://image-component.nextjs.gallery/layout-fixed)
- [Demostración de `intrinsic`](https://image-component.nextjs.gallery/layout-intrinsic)
- [Demostración de `responsive`](https://image-component.nextjs.gallery/layout-responsive)
- [Demostración de `fill`](https://image-component.nextjs.gallery/layout-fill)
- [Demostración de una imagen fondo](https://image-component.nextjs.gallery/background)
...
la mejor calidad. Por defecto es 75.
...
Qué mostrar mientras la imagen se carga. Los valores posibles son `blur` y
`empty`. Por defecto es `empty`.

Cuando es `blur`, se mostrará una versión difuminada de la imagen hasta
que esta carge. Esta versión difuminada es dada por la propiedad
[`blurDataURL`](#blurdataurl). Si la imagen en cuestión fue importada
estáticamente y es de tipo JPG, PNG o WebP, entonces tendrá el valor
`blurDataURL` cargado automáticamente.

Para imágenes dinámicas, debes proporcionar la propiedad
[`blurDataURL`](#blurdataurl) manualmente. Soluciones como
[Plaiceholder](https://github.com/joe-bell/plaiceholder) pueden ayudar con
la generación en `base64`.

Cuando es `empty`, se mostrará un espacio vacío con las dimensiones de la
imagen mientra esta carga.

Pruébalo:

- [Demostración de `blur`](https://image-component.nextjs.gallery/placeholder)
- [Demostración de un efecto reluciente en `blurDataURL`](https://image-component.nextjs.gallery/shimmer)
...
[Más información](https://developer.mozilla.org/en-US/docs/Web/CSS/object-fit).

#### objectPosition

Cómo debe posicionarse una imagen cuando `layout="fill"`.

[Más información](https://developer.mozilla.org/en-US/docs/Web/CSS/object-position).
...
Una
[URL de datos](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URIs)
utilizada como un _placeholder_ antes de que la imagen carge
satisfactoriamente. Solo funciona cuando
[`placeholder="blur"`](#placeholder).

Debe ser una imagen codificada en base64. Será ampliada y difuminada, por
lo que se recomienda utilizar una imagen muy pequeña (10px o menos).
Incluir imágenes más grandes podría dañar el rendimiento de tu aplicación.

Pruébalo:

- [Demostración de la propiedad `blurDataURL` por defecto](https://image-component.nextjs.gallery/placeholder)
- [Demostración de un efecto reluciente en `blurDataURL`](https://image-component.nextjs.gallery/shimmer)

También puedes
[generar un URL de datos que sea un color sólido](https://png-pixel.com)
que corresponda a la imagen.

#### unoptimized

Cuando es `true`, la imagen original será servida tal cual como vino al
mundo en vez de alterar su calidad, dimensiones y/o formato. Por defecto
es `false`.
