...
function Home() {
  return (
    <>
      <h1>Mi Página Web</h1>
      <Image src={profilePic} alt="Foto del autir" />
...
`import`. (Nota: `require` no está soportado—solo `import`.)
  ...
  return (
    <>
      ...
      <Image
        ...
        //
        // Opcionalmente permite añadir una versión difuminada de
        // la imagen mientras esta carga
        // placeholder="blur"
      />
...
Un _placeholder_ para mostrar mientras la imagen se carga. Los valores
posibles son `blur` y `empty`. Por defecto es `empty`.

Cuando es `blur`, la propiedad [`blurDataURL`](#blurdataurl) será
utilizada como un _placeholder_. Si la imagen en cuestión fue importada
estáticamente y esta es un JPG, PNG o WebP, entonces tendrá el valor
...
Cuando es `empty`, no habrá ningún _placeholder_ mientra la imagen carga,
solo espacio vacío.

Pruébalo:

- [Demo del _placeholder_ `blur`](https://image-component.nextjs.gallery/placeholder)
- [Demo de un efecto reluciente en `blurDataURL`](https://image-component.nextjs.gallery/shimmer)
...
- [Demo de la propiedad `blurDataURL` por defecto](https://image-component.nextjs.gallery/placeholder)
- [Demo de un efecto reluciente en `blurDataURL`](https://image-component.nextjs.gallery/shimmer)
