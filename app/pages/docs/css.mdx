...
Blitz te permite import archivos de CSS directamente en cualquier archivo
de JavaScript.
...
```css
/*
¡No tienes que precuparte de que .error {} colisione con otro en algún
`.css` o `.module.css`!
*/
.error {
  color: white;
...
export function Button() {
  return (
    <button
      type="button"
      // Fíjate como la clase "error" es accedida como una propiedad
...
Blitz te permite imporar Sass, ya sea en el formato `.scss` o `.sass`. Los
módulos CSS también funcionan con Sass, utilizando las extensiones
...
Para poder imporar archivos `.less` o `.styl`, puedes utilizar los
siguientes paquetes:
...
Si usas quiere soporte para Less, no te olvides de añadir `less` como
dependencia, de lo contrario te saldrá un error como este:
